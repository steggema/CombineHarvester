...
parser.add_argument(
    '--output', '-o',default = "./mapped_coupling_from_width.txt", help="""Output file name""")
parser.add_argument(
...
for mass in masses:
 for width in widths:
  ...
  try:
   tanb = min(InverseInterpolate(arr,width_GeV)) #tanb is interpolated along fixed mass to find tanb closest to the width
   _,arr = TH2FixAxisPlot(h2,0.1,Xfix=mass)
   g = Interpolate(arr,tanb)
   dat.append([mass,width_GeV,g]) 
  except:
   set_trace()

out = open(args.output,"w")
out.write("# mass(GeV) width(GeV) g\n")
for elem in dat:
