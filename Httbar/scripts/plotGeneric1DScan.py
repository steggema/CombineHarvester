...
def BuildScan(scan, param, files, color, yvals, ycut):
    ...
    for yval in yvals:
        ...
        if not bestfit:
            for crossing in crossings[yval]:
                if crossing['hi'] - crossing['lo'] > best_dist:
                    bestfit = 0.5*(crossing['hi'] + crossing['lo'])
                    best_dist = crossing['hi'] - crossing['lo']
...
parser.add_argument('--y-cut', type=float, default=1., help='Remove points with y > y-cut')
parser.add_argument('--y-max', type=float, default=1.5, help='y-axis maximum')
...
axishist.GetYaxis().SetTitle("CLs")
# axishist.GetXaxis().SetTitle("%s" % fixed_name)
axishist.GetXaxis().SetTitle("Coupling modifier")
...
for yval in yvals:
    ...
    if (len(other_scans) == 0):
        for cr in main_scan['crossings'][yval]:
            ...
            line.DrawLine(cr['hi'], 0, cr['hi'], yval) # So we only expect one crossing
