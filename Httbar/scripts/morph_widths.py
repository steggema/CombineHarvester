...
# set_trace()
print 'FIXME - determine the range of widths to be morphed'
new_points = dictionary = dict(zip(list(np.arange(2.5, 50, 0.5)), list("%s%s" % (
    str(x).replace('.', 'p').replace('p0', ''), "pc") for x in np.arange(2.5, 50, 0.5))))
...
if len(intersection) and not args.forchecks:
    raise ValueError('the points: %s are already computed, I will not do them again!' %
...
for point in to_make:
    ...
    if not above or not below:
        raise ValueError(
            'I cannot interpolate %s as it is outside the provided range!' % point)
...
for category in [i.GetName() for i in infile.GetListOfKeys()]:
    ...
    shapes = {"sgn": (i for i in shapes if '_pos-sgn' in i),
              "int": (i for i in shapes if '-int-' in i)}
